@php
    ...
    $donateableItems = in_array('any', $data['item_ids']) ? \App\Models\Item\Item::all() : 
        (isset($data['item_category_ids']) ? 
            \App\Models\Item\Item::whereIn('item_category_id', $data['item_category_ids'])->whereIn('id', $data['item_ids'] ?? []) : 
            \App\Models\Item\Item::with('rarity')->whereIn('id', $data['item_ids'] ?? []));
    $donateableItems = $donateableItems->get()->groupBy(function($item) {
